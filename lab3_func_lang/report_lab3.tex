...
\begin{itemize}[nosep]
  ...
  \item дополнительный язык для сравнения: Python 3;
...
% РАЗДЕЛ 4
% =========================================================
\section{РЕАЛИЗАЦИЯ ЗАДАНИЙ НА ИМПЕРАТИВНОМ ЯЗЫКЕ PYTHON}

Для сравнения функционального и императивного подходов те же задания были реализованы на языке Python. В Python-версии используются обычные переменные, условные операторы \texttt{if}/\texttt{elif}/\texttt{else} и циклы, что соответствует императивному стилю программирования.

\subsection{Задание 2.1~-- вычисление математического выражения}

\begin{lstlisting}[caption={Задание 2.1 на Python (task2\_1.py)}, language=Python]
import math
...
\subsection{Задание 2.2~-- функция Y(x)}

\begin{lstlisting}[caption={Задание 2.2 на Python (task2\_2.py)}, language=Python]
import math
...
\subsection{Задание 2.3~-- вычисление факториала}

\begin{lstlisting}[caption={Задание 2.3 на Python (task2\_3.py)}, language=Python]
def factorial(n):
    if n < 0:
...
\subsection{Задание 3~-- обработка текста}

\begin{lstlisting}[caption={Задание 3 на Python (task3\_text.py)}, language=Python]
...
Реализации на Common Lisp и Python дают одинаковые результаты на выбранных тестовых данных, однако отличаются способом описания вычислений.

\begin{table}[H]
\centering
\caption{Сравнение реализаций на Common Lisp и Python}
\begin{tabularx}{\textwidth}{|p{0.24\textwidth}|X|X|}
\hline
\textbf{Критерий} & \textbf{Common Lisp} & \textbf{Python} \\
\hline
Математические выражения & Записываются в префиксной форме: операция указывается перед аргументами. & Записываются в привычной инфиксной форме, близкой к математической записи. \\
\hline
Условные конструкции & Используется \texttt{cond}, где каждая ветвь содержит условие и выражение-результат. & Используются операторы \texttt{if}, \texttt{elif}, \texttt{else}, которые выполняют последовательные команды. \\
\hline
Повторение действий & В задании факториал реализован через хвостовую рекурсию с аккумулятором. & Факториал реализован через цикл \texttt{for} и изменяемую переменную \texttt{result}. \\
\hline
Обработка текста & Список предложений преобразуется функционалами \texttt{mapcar} и \texttt{reduce}. & Предложения обрабатываются последовательно в цикле, результат накапливается в списке. \\
\hline
Стиль программы & Акцент сделан на вычислении значений выражений и применении функций. & Акцент сделан на последовательности действий и изменении промежуточного состояния. \\
\hline
\end{tabularx}
\end{table}

Наиболее заметное отличие проявляется в задании 2.3. В Common Lisp факториал описывается рекурсивно: функция передает новое значение аккумулятора в следующий вызов. В Python та же задача естественно выражается циклом, где переменная \texttt{result} изменяется на каждой итерации.

В задании 3 функциональная реализация показывает применение функций высшего порядка: одна функция применяется ко всем предложениям, а затем список результатов сворачивается в строку. Императивная Python-версия явно проходит по предложениям, проверяет каждую строку и добавляет обработанный результат в список. Такой вариант проще воспринимается как последовательный алгоритм, но сильнее зависит от изменяемого состояния.
...
В данном разделе приведены экранные формы, отображающие результаты выполнения программ в среде SBCL (запуск через \texttt{sbcl --script}) и Python (запуск через \texttt{python3}).
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.85\textwidth]{screenshot_task21_python.png}
  \caption{Результат выполнения задания 2.1 на Python}
  \label{fig:task21-python}
\end{figure}

\begin{figure}[H]
  \centering
  \includegraphics[width=0.85\textwidth]{screenshot_task22_python.png}
  \caption{Результат выполнения задания 2.2 на Python}
  \label{fig:task22-python}
\end{figure}

\begin{figure}[H]
  \centering
  \includegraphics[width=0.85\textwidth]{screenshot_task23_python.png}
  \caption{Результат выполнения задания 2.3 на Python}
  \label{fig:task23-python}
\end{figure}

\begin{figure}[H]
  \centering
  \includegraphics[width=0.85\textwidth]{screenshot_task3_python.png}
  \caption{Результат выполнения задания 3 на Python}
  \label{fig:task3-python}
\end{figure}
...
6 Выполнена дополнительная реализация заданий на императивном языке Python с использованием условных операторов, циклов и изменяемых переменных.

7 Проведено сравнение функционального и императивного подходов. Установлено, что оба варианта дают одинаковые результаты на тестовых данных, но отличаются способом организации вычислений: Common Lisp ориентирован на применение функций и рекурсию, а Python~-- на последовательность команд и явное изменение состояния.

8 Изучены основные конструкции языка Common Lisp: определение функций, условные выражения, рекурсия, функции высшего порядка, работа со строками и списками.
...
\begin{enumerate}[label=\arabic*,leftmargin=1.25cm]
  ...
  \item Python 3 Documentation [Электронный ресурс].~-- Режим доступа: \url{https://docs.python.org/3/}.~-- Дата доступа: 03.06.2026.
\end{enumerate}
